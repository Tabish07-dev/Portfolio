...
import { HiMail, HiPhone, HiLocationMarker, HiPaperAirplane, HiClipboard, HiCheck } from 'react-icons/hi';
...
const Contact: React.FC = () => {
  ...
  return (
    <section id="contact" className="section-padding bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
          ...
          >
            ...
            <div className="space-y-4">
              {contactInfo.map((info) => (
                <div
                  key={info.id}
                  className="glass-card p-5 flex items-start justify-between gap-4 hover:shadow-lg hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 group"
                >
                  <a href={info.link} className="flex items-start gap-4 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-base font-semibold text-slate-950 dark:text-white mt-0.5 break-all">
                        {info.value}
                      </p>
                    </div>
                  </a>
                  ...
                  {/* Clipboard Action */}
                  <button
                    onClick={() => handleCopy(info.copyValue, info.id)}
                  ...
                  >
                    {copiedId === info.id ? <HiCheck className="h-4 w-4 text-emerald-500" /> : <HiClipboard className="h-4 w-4" />}
                    {copiedId === info.id && (
                ...
                {socialLinks.map((social) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      title="Copy URL"
                    >
                      {copiedId === social.id ? <HiCheck className="h-4 w-4 text-emerald-500" /> : <HiClipboard className="h-4 w-4" />}
                      {copiedId === social.id && (
                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] px-2 py-0.5 rounded shadow">
                          Copied!
                        </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    required
                    className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-950 dark:text-white focus:outline-none focus:border-blue-500 transition-colors"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  required
                  rows={4}
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-950 dark:text-white focus:outline-none focus:border-blue-500 transition-colors resize-none"
              ...
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 disabled:bg-slate-300 dark:disabled:bg-slate-800 disabled:cursor-not-allowed text-white font-medium py-3 px-6 rounded-xl transition-all duration-300 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
